...
import { ArrowLeft, Calendar, Clock, MapPin, Users, Edit, Send } from "lucide-react";
import { eventStorage, guestStorage, Event, Guest } from "@/lib/storage";
  ...
  useEffect(() => {
    if (eventId) {
      try {
        ...
        const guestData = guestStorage.getByEventId(eventId);
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span>{event.confirmedGuests}/{event.maxGuests} invitados</span>
            </div>
            <div className="w-full bg-muted rounded-full h-2 mt-2">
              <div 
                className="bg-primary h-2 rounded-full" 
                style={{ width: `${(event.confirmedGuests / event.maxGuests) * 100}%` }}
              ></div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {["confirmed", "pending", "declined"].map((status) => {
                ...
                return (
                  <div key={status}>
                    <h4 className={`font-medium mb-2 ${statusColors[status as keyof typeof statusColors]}`}>
                      {statusLabels[status as keyof typeof statusLabels]} ({statusGuests.length})
                    </h4>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {new Date(event.createdAt).toLocaleDateString('es-CO')} a las{" "}
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {new Date(event.updatedAt).toLocaleDateString('es-CO')} a las{" "}
                {new Date(event.updatedAt).toLocaleTimeString('es-CO')}
              </p>
            </div>
          </div>
        </CardContent>
