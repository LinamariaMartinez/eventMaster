...
import { generateShortId, generatePublicUrl } from "@/lib/utils";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Generar nuevo public_url único (ID corto de 8 caracteres)
    const shortId = generateShortId();
    const publicUrl = generatePublicUrl(shortId);
    ...
    const { data: newEvent, error: createError } = await (
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      supabase.from("events").insert as any
    )(newEventData)
      .select()
      .single();
