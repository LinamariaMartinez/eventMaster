...
const mockReminders: AutomatedReminder[] = [
  ...
  },
  {
    ...
    lastRun: "2025-01-20T16:00:00Z",
    nextRun: "2025-01-21T16:00:00Z",
    totalSent: 89,
  },
  {
    id: "3",
...
export function AutomatedReminders() {
  const [reminders, setReminders] =
    useState<AutomatedReminder[]>(mockReminders);
  const [showCreateDialog, setShowCreateDialog] = useState(false);
  const [editingReminder, setEditingReminder] =
    useState<AutomatedReminder | null>(null);
  ...
  const handleCreateReminder = (
    reminderData: Omit<AutomatedReminder, "id" | "totalSent">,
  ) => {
  ...
  const getTriggerLabel = (
    trigger: AutomatedReminder["trigger"],
    timing: number,
  ) => {
    switch (trigger) {
      case "days_before":
  ...
  const getNotificationTypeIcon = (
    type: AutomatedReminder["notificationType"],
  ) => {
    switch (type) {
      case "email":
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configura recordatorios automáticos para mantener a tus invitados
            informados.
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {reminders.reduce((sum, r) => sum + r.totalSent, 0)}
                </p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {reminders.filter((r) => r.nextRun).length}
                </p>
                  ...
                  <div className="text-right">
                    ...
                    {reminder.nextRun && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Próximo: {formatDate(reminder.nextRun)}
                      </p>
                    )}
                  </div>
...
function ReminderForm({ initialData, onSubmit, onCancel }: ReminderFormProps) {
  const [formData, setFormData] = useState({
    ...
    trigger:
      initialData?.trigger || ("days_before" as AutomatedReminder["trigger"]),
    timing: initialData?.timing || 1,
    eventTypes: initialData?.eventTypes || ["all"],
    notificationType:
      initialData?.notificationType ||
      ("email" as AutomatedReminder["notificationType"]),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="rsvp_deadline">
                Antes de fecha límite RSVP
              </SelectItem>
              <SelectItem value="no_response">
                Sin respuesta por X días
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          <Label htmlFor="timing">
            {formData.trigger.includes("hours") ? "Horas" : "Días"}
          </Label>
          <Input
            id="timing"
            type="number"
            ...
            onChange={(e) =>
              setFormData((prev) => ({
                ...prev,
                timing: Number.parseInt(e.target.value) || 1,
              }))
            }
            required
          />
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="onlyUnconfirmed">
              Solo enviar a invitados sin confirmar
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <Switch
            ...
            />
            <Label htmlFor="excludeDeclined">
              Excluir invitados que rechazaron
            </Label>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit">
          {initialData ? "Actualizar" : "Crear"} Recordatorio
        </Button>
      </div>
    </form>
  );
