...
import { Plus, Edit, Trash2, MoreHorizontal, Mail, Bell, Smartphone, Copy } from "lucide-react"

interface NotificationTemplate {
  id: string
  name: string
  type: "email" | "push" | "sms"
  category: "event_reminder" | "rsvp_update" | "guest_added" | "deadline" | "custom"
  subject: string
  content: string
  variables: string[]
  isDefault: boolean
...
const mockTemplates: NotificationTemplate[] = [
  ...
  },
  {
    ...
    content: "No olvides que {event_name} comienza a las {event_time} en {event_location}",
  ...
  },
  {
    ...
    category: "deadline",
    subject: "",
    content: "URGENTE: La fecha límite para {event_name} vence hoy. Confirma tu asistencia: {rsvp_link}",
...
export function NotificationTemplates() {
  ...
  const handleCreateTemplate = (templateData: Omit<NotificationTemplate, "id" | "createdAt">) => {
  ...
  const handleEditTemplate = (templateData: Omit<NotificationTemplate, "id" | "createdAt">) => {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground mb-2">Plantillas de Notificación</h2>
          <p className="text-muted-foreground">Crea y gestiona plantillas reutilizables para tus notificaciones.</p>
        </div>
...
function TemplateForm({ initialData, onSubmit, onCancel }: TemplateFormProps) {
  const [formData, setFormData] = useState({
    ...
    category: initialData?.category || ("event_reminder" as NotificationTemplate["category"]),
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Select
            value={formData.type}
            onValueChange={(value: NotificationTemplate["type"]) => setFormData((prev) => ({ ...prev, type: value }))}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
